...
def conv_block_sn(in_ch, out_ch, k=3, s=1, p=1, norm=True, dropout=0.0):
    layers = [nn.utils.spectral_norm(nn.Conv2d(in_ch, out_ch, k, s, p, bias=False))]
    if norm:
...
def deconv_block(in_ch, out_ch, k=3, s=1, p=1, norm=True, dropout=0.0):
    layers = [
        nn.Upsample(scale_factor=2, mode='nearest'), # <- antes: bilinear
...
class InpaintDiscriminator(nn.Module):
    ...
    def __init__(self, in_ch_cond=4, in_ch_img=3, base=64, dropout=0.0):
        ...
        # Aumentamos profundidad para que perciba detalles (ojos, pelo)
        self.net = nn.Sequential(
            ...
            conv_block(base*4, base*8, k=4, s=2, p=1),                  # 16→8
            conv_block(base*8, base*8, k=4, s=1, p=1, dropout=dropout), # RF ↑
...
class InpaintDiscriminator_SN(nn.Module):
    """
    Discriminador condicional tipo PatchGAN con SpectralNorm.
    Entrada: imagen condicional (4ch) + imagen completa (3ch) → 7ch.
    """
    def __init__(self, in_ch_cond=4, in_ch_img=3, base=64, dropout=0.0):
        super().__init__()
        in_ch_total = in_ch_cond + in_ch_img  # 7 canales totales
        ...
        self.net = nn.Sequential(
            conv_block_sn(in_ch_total, base, k=4, s=2, p=1, norm=False),
            conv_block_sn(base, base*2, k=4, s=2, p=1),
            conv_block_sn(base*2, base*4, k=4, s=2, p=1),
            conv_block_sn(base*4, base*8, k=4, s=2, p=1, dropout=dropout),
