...
from models_07 import InpaintUNet2 as UNetGenerator
from models_07 import InpaintDiscriminator as Discriminator
...
with open(f"{run_dir}/config.txt", "w") as f:
    f.write(f"""Fecha: {run_id}
        ...
        Loss = LSGAN (MSE) + L1(hole, valid) + perceptual(VGG16) + edge(Sobel)
        """)
...
# -----------------------------------------------------
# Pérdida de gradiente (edge-aware) solo en el hueco
# -----------------------------------------------------

# Refuerza contornos (ojos/bigotes) sin pelearse con L1.

def grad_loss(x, y, mask):

    def sobel(img):
        dtype = img.dtype
        device = img.device
        gx = F.conv2d(img, torch.tensor([[[[-1,0,1],[-2,0,2],[-1,0,1]]]], dtype=dtype, device=device).repeat(3,1,1,1),
                      padding=1, groups=3)
        gy = F.conv2d(img, torch.tensor([[[[-1,-2,-1],[0,0,0],[1,2,1]]]], dtype=dtype, device=device).repeat(3,1,1,1),
                      padding=1, groups=3)
    ...
    gx = sobel(x)
    gy = sobel(y)
    return F.l1_loss(gx * hole, gy * hole)
    ...
    for cond, gt, mask in loop:
        ...
        if epoch <= WARMUP_EPOCHS:
            ...
            loss_G = (LAMBDA_ADV*adv_loss
                    ...
                    + LAMBDA_EDGE*grad_loss(pred, gt, mask))
