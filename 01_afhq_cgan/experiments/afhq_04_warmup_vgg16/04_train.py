...
from models_04 import InpaintUNet as UNetGenerator
from models_04 import InpaintDiscriminator as Discriminator
...
LAMBDA_ADV   = 0.004
LAMBDA_HOLE  = 4.0       # un poco menos, para no dominar
LAMBDA_VALID = 1.0
LAMBDA_VGG   = 0.02      # + detalle visual
DROPOUT      = 0.1       # elimina suavizado innecesario
...
# Fijar semilla para que los entrenamientos sean comparables
...
# cada peso de cada conv o batchnorm se inicializa como un número aleatorio con:
# - media = 0
...
# Adam es el optimizador que actualiza los pesos de la red en cada iteración del entrenamiento.
# Cada peso se ajusta usando una media móvil de su gradiente y de su cuadrado (como “momentum adaptativo”).
# lr → la tasa de aprendizaje base.
# β1=0.5, β2=0.999 → configuración “DCGAN” clásica
...
def plot_losses(save_path):
    ...
    plt.plot(log["D_real_mean"], label="D Real Mean", color="purple")
    plt.plot(log["D_fake_mean"], label="D Fake Mean", color="brown")

    # Añadido para ver la norma de los gradientes
    plt.twinx()
    plt.plot(log["grad_G"], label="Grad G", color="cyan", linestyle="--", alpha=0.6)
    plt.plot(log["grad_D"], label="Grad D", color="magenta", linestyle="--", alpha=0.6)
    plt.ylabel("Norma del gradiente (log)")
    plt.yscale("log")  # opcional, ayuda a ver variaciones grandes

    plt.xlabel("Época"); plt.ylabel("Pérdida media"); plt.legend()
    plt.tight_layout(); plt.savefig(save_path); plt.close()

# ----------------------------------------------
# 5) Batch y máscara fijos para visualización
# ----------------------------------------------
# Fija un batch y máscara para seguimiento visual
...
# Crear una máscara fija para cada imagen del fixed_batch
fixed_mask = torch.stack([
    random_circle_mask(IMG_SIZE, min_radius=12, max_radius=40)
    for _ in range(fixed_batch.size(0))
]).to(DEVICE)                               # [B,1,H,W]

# Construir la condición fija: RGB enmascarado + canal de máscara
fixed_cond_rgb = fixed_batch * fixed_mask   # borra el hueco (mask=0)
fixed_cond = torch.cat([fixed_cond_rgb, fixed_mask], dim=1)  # [B,4,H,W]

# --------------------------------------------
# 6) Perceptual loss usando VGG16 preentrenada
# --------------------------------------------
from torchvision.models import vgg16
...
for epoch in range(1, EPOCHS+1):
    ...
    for cond, gt, mask in loop:
        ...
        # --------- Forward G ---------
        pred = G(cond)                          # (B,3,H,W) imagen generada completa
        comp_fake = pred*(1-mask) + gt*mask     # compuesta: fake en el hueco, real fuera
    ...
    with torch.no_grad():
        ...
        pred_f = G(fixed_cond)
        comp_f = pred_f * (1 - fixed_mask) + fixed_batch * fixed_mask

        # Creamos el grid: cond (RGB), mask (visualizada en gris), pred, comp, gt
        grid = torch.cat([
            fixed_cond[:, :3],
            fixed_mask.repeat(1, 3, 1, 1) * 2 - 1,
