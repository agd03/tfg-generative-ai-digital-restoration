...
from models_03 import InpaintUNet as UNetGenerator
from models_03 import InpaintDiscriminator as Discriminator
from mask_utils import random_circle_mask
from utils import weights_init_dcgan


# ============================================================
# Configuración reconstruida para exp05 / 20251106_223730
# ============================================================
...
# Aparece en config.txt, pero NO se usa en la loss de exp05.
# La línea del config indica: Loss=LSGAN (MSE) + L1(hole, valid)
LAMBDA_VGG = 0.01
...
def plot_losses(save_path):
    ...
    # En el run original aparece como "Época", aunque realmente hay
    # muchos más puntos porque se registra por iteración/batch.
    plt.xlabel("Época")
...
def make_mask_batch(batch_size):
    masks = []
...
print("Entrenando Inpainting-GAN exp05...")
...
for epoch in range(1, EPOCHS + 1):
    ...
    for cond, gt, mask in loop:
        ...
        # Pérdida adversarial solo sobre el hueco,
        # adaptando la máscara al mapa de salida del discriminador.
        ...
        # L1 sobre hueco y contexto visible
        l1_hole = F.l1_loss(pred * (1 - mask), gt * (1 - mask))
        l1_valid = F.l1_loss(pred * mask, gt * mask)

        # Exp05: LSGAN + L1. No se usa VGG aunque LAMBDA_VGG esté en config.
        loss_G = (
            LAMBDA_ADV * adv_loss
            + LAMBDA_HOLE * l1_hole
        ...
        loop.set_postfix({
            ...
            "L1_h": f"{l1_hole.item():.3f}",
            "L1_v": f"{l1_valid.item():.3f}",
        })
    ...
    with torch.no_grad():
        ...
        # En exp05 se aprecia batch fijo, pero máscaras variables por época.
        mask_f = make_mask_batch(b).to(DEVICE)

        cond_rgb = fixed_batch * mask_f
