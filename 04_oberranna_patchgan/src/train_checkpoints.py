...
def evaluate(G, D, loader, device, vgg_feat=None, lpips_fn=None):
    ...
    for cond, gt, mask in loader:
        ...
        loss_L1_boundary = l1_boundary_loss(fake, gt, hole, kernel_size=3)
...
def save_full_ckpt(path, epoch, G, D, optim_G, optim_D, history, best_metric, best_epoch):
    torch.save({
        "epoch": epoch,
        "G": G.state_dict(),
        "D": D.state_dict(),
...
def load_full_ckpt(path, device, G, D, optim_G=None, optim_D=None, load_optim=True):
    ckpt = torch.load(path, map_location=device)
    G.load_state_dict(ckpt["G"])
    D.load_state_dict(ckpt["D"])
    if load_optim and optim_G is not None and optim_D is not None:
        optim_G.load_state_dict(ckpt["optim_G"])
        optim_D.load_state_dict(ckpt["optim_D"])
    history = ckpt.get("history", None)
    best_metric = ckpt.get("best_metric", float("inf"))
    best_epoch = ckpt.get("best_epoch", -1)
    start_epoch = ckpt["epoch"] + 1  # siguiente epoch
    return start_epoch, history, best_metric, best_epoch
...
def train(
    ...
    # Para reanudar entrenamientos (opcional)
    ...
    if resume:
        if resume_which == "last":
            ckpt_path = os.path.join(ckpt_dir, "ckpt_last.pt")
        elif resume_which == "best":
            ckpt_path = os.path.join(ckpt_dir, "ckpt_best.pt")
        else:
            assert resume_epoch is not None, "--resume_epoch es obligatorio si resume_which=epoch"
            ckpt_path = os.path.join(ckpt_dir, f"ckpt_epoch_{resume_epoch:03d}.pt")

        if os.path.exists(ckpt_path):
            start_epoch, hist2, best_metric, best_epoch = load_full_ckpt(
                ckpt_path, device, G, D, optim_G, optim_D, load_optim=(not reset_optim)
            )
            if hist2 is not None:
                history = hist2
            print(f"✅ Reanudando desde {ckpt_path} en epoch {start_epoch+1}")
        else:
            # fallback: reanudar solo pesos si no existe ckpt_full
            g_path = os.path.join(ckpt_dir, "G_last.pth" if resume_which=="last" else "G_best.pth")
            d_path = os.path.join(ckpt_dir, "D_last.pth" if resume_which=="last" else "D_best.pth")
            ...
            print(f"⚠️ No existe {ckpt_path}; cargados pesos {g_path} / {d_path}, optimizadores reiniciados.")
            start_epoch = 0
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for step, (cond, gt, mask) in enumerate(progress_bar):
            ...
            loss_L1_boundary = l1_boundary_loss(fake_G, gt, hole, kernel_size=3)
        ...
        # Para poder reanudar con optimizadores e historial, guardamos un checkpoint completo con todo el estado

        save_full_ckpt(os.path.join(ckpt_dir, "ckpt_last.pt"),
               epoch, G, D, optim_G, optim_D, history, best_metric, best_epoch)

        # y por epoch
        save_full_ckpt(os.path.join(ckpt_dir, f"ckpt_epoch_{epoch+1:03d}.pt"),
                    epoch, G, D, optim_G, optim_D, history, best_metric, best_epoch)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mask_blur_radius", type=float, default=3.0)
    ...
    # Para reanudar entrenamiento desde un checkpoint
    parser.add_argument("--run_dir", type=str, default=None,
                        help="Si se especifica, usa este directorio en lugar de crear uno nuevo (útil para resume).")
    parser.add_argument("--resume", action="store_true",
                        help="Reanuda desde un checkpoint dentro de run_dir.")
    parser.add_argument("--resume_which", type=str, default="last",
                        choices=["last", "best", "epoch"],
                        help="Qué checkpoint cargar: last, best o epoch.")
    parser.add_argument("--resume_epoch", type=int, default=None,
                        help="Si resume_which=epoch, qué epoch cargar (ej. 25).")
    parser.add_argument("--reset_optim", action="store_true",
                        help="Si se pasa, NO carga optimizadores al reanudar (útil si cambias lambdas).")
